...
/** 
 *  SourceUnit: /Users/ildar/work/unlimit/evm-contracts/contracts/LiquidityPool.sol
*/
...
interface IExchangeCore {
    ...
    function lockSource(HtlcLockSource calldata params) external;
...
interface IProtocolAdmin {
    function setLiquidityThresholds(address token, uint256 X, uint256 Y, uint256 Z) external;

    function whitelistToken(address token, bool enable) external;
}
...
////import { IExchangeCore } from "./IExchangeCore.sol";
...
interface ILiquidityPool {
    ...
    function singleChainSwap(SwapLocal calldata params, bytes calldata maintainerSig) external;
    ...
    function lockSource(IExchangeCore.Base calldata base, uint256 nonce, uint64 untilTs) external;

    function lockSourceWithPermit(
        IExchangeCore.Base calldata base,
        uint256 nonce,
        uint64 untilTs,
...
////import { IProtocolAdmin } from "./interfaces/IProtocolAdmin.sol";
    ...
    IProtocolAdmin
{
    ...
    bytes32 public constant CROSS_CHAIN_MODULE_ROLE = keccak256("CROSS_CHAIN_MODULE_ROLE");
    bytes32 public constant BACKEND_ROLE = keccak256("BACKEND_ROLE");

    // ========= EIP-712 =========
    bytes32 private constant SWAP_LOCAL_TYPEHASH =
        keccak256(
            "SwapLocal(uint256 amountIn,address tokenIn,address tokenOut,uint64 chainId,address recipient,uint64 deadline,address user,uint256 nonce)"
        );
    ...
    mapping(bytes32 => mapping(address => uint256)) public lockedAmount; // secretHash => token => amount
    ...
    mapping(address => uint256) public protocolFeeByToken; // token => fee amount
    mapping(address => bool) public feeEnabledByToken; // token => enabled fee

    // ========= Events =========
    event TokenWhitelisted(address token, bool enabled, uint8 decimals);
    ...
    event ReleasedToRecipient(bytes32 secretHash, address token, address to, uint256 amount);
    event FinalizedToPool(bytes32 secretHash, address token, uint256 amount);
    event MaintainerSet(address maintainer);
    event LiquidityThresholdsUpdated(address token, uint256 X, uint256 Y, uint256 Z);
    event ProtocolFeeForTokenUpdated(address token, uint256 feeAmount);
    event ProtocolFeeEnabledForTokenUpdated(address token, bool enabled);
    event ProtocolFeeCollected(address token, uint256 feeAmount);
    ...
    function setProtocolFeeForToken(
        ...
        protocolFeeByToken[token] = feeAmount;
        emit ProtocolFeeForTokenUpdated(token, feeAmount);
    }
    ...
    function setLiquidityThresholds(
        ...
        emit LiquidityThresholdsUpdated(token, X, Y, Z);
    }

    function whitelistToken(address token, bool status) external onlyRole(POOL_MANAGER_ROLE) {
        isWhitelisted[token] = status;
        if (status) {
            uint8 dec = IERC20Metadata(token).decimals();
            tokenDecimals[token] = dec;
            emit TokenWhitelisted(token, true, dec);
        } else {
            emit TokenWhitelisted(token, false, tokenDecimals[token]);
        }
    }
    ...
    function withdrawLiquidity(
        ...
        uint256 available = IERC20(token).balanceOf(address(this)) - totalLockedByToken[token];
    ...
    function singleChainSwap(
        ...
        _executeSwap(params, maintainerSig, msg.sender);
    }
    ...
    function singleChainSwapWithPermit(
        ...
        uint256 permitAmount = params.base.amountIn + params.fee;
        IERC20Permit(params.base.tokenIn).permit(
            params.user,
            address(this),
            permitAmount,
    ...
    function singleChainSwapWithPermit2(
        ...
        uint256 permit2Amount = params.base.amountIn + params.fee;
        IAllowanceTransfer(permit2).permit(params.user, permitSingle, params.permit2Signature);
    ...
    function lockSource(
        ...
        uint256 nonce,
        uint64 untilTs
    ) external onlyRole(CROSS_CHAIN_MODULE_ROLE) nonReentrant whenNotPaused {
    ...
    function lockSourceWithPermit(
        ...
        IERC20Permit(base.token).permit(
            base.user,
            address(this),
            permitAmount,
            permitData.deadline,
    ...
    function lockSourceWithPermit2(
        ...
        IAllowanceTransfer(permit2).permit(base.user, permitSingle, permit2Data.permit2Signature);
        IAllowanceTransfer(permit2).transferFrom(
            base.user,
            address(this),
            uint160(permit2Amount),
    ...
    function _validateAndUpdateNonce(address user, uint256 nonce) internal {
        if (nonce <= lastNonce[user]) revert InvalidNonce();
    ...
    function _lockSource(
        ...
        lockedAmount[secretHash][token] += amount;
    ...
    function _executeSwap(
        ...
        if (params.chainId != block.chainid) revert InvalidChainId();
    ...
    function _applyFee(address token, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = protocolFeeByToken[token];
        if (feeAmount == 0) return amount;

        if (amount <= feeAmount) revert AmountTooSmallForFee();

        emit ProtocolFeeCollected(token, feeAmount);
